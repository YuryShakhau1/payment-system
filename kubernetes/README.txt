...
2.
Add SSL secret
New-Item -ItemType Directory -Force -Path "D:\certs"
cd D:\certs
mkcert -install
mkcert payment-system.local api.payment-system.local
kubectl create secret tls local-ssl-secret --cert=payment-system.local+1.pem --key=payment-system.local+1-key.pem
curl.exe -v --ssl-no-revoke https://payment-system.local
curl.exe -v --ssl-no-revoke https://api.payment-system.local
